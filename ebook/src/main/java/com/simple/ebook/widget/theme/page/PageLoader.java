...
import com.simple.ebook.helper.BookRecordHelper;
...
public abstract class PageLoader {
    ...
    private void initData() {
        mSettingManager = ReadSettingManager.getInstance(mContext);
        mTextSize = mSettingManager.getTextSize();
        mTitleSize = mTextSize + ScreenUtils.spToPx(mContext, EXTRA_TITLE_SIZE);
    ...
    public void setBgColor(int theme) {
        if (isNightMode && theme == ReadSettingManager.NIGHT_MODE) {
            mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_fff_99);
            mPageBg = ContextCompat.getColor(getAppContext(), R.color.black);
        } else if (isNightMode) {
            mBgTheme = theme;
            ...
            switch (theme) {
                case ReadSettingManager.READ_BG_DEFAULT:
                    mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_2c);
                    mPageBg = ContextCompat.getColor(getAppContext(), R.color.color_cec29c);
                    break;
                case ReadSettingManager.READ_BG_1:
                    mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_2f332d);
                    mPageBg = ContextCompat.getColor(getAppContext(), R.color.color_ccebcc);
                    break;
                case ReadSettingManager.READ_BG_2:
                    mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_92918c);
                    mPageBg = ContextCompat.getColor(getAppContext(), R.color.color_aaa);
                    break;
                case ReadSettingManager.READ_BG_3:
                    mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_383429);
                    mPageBg = ContextCompat.getColor(getAppContext(), R.color.color_d1cec5);
                    break;
                case ReadSettingManager.READ_BG_4:
                    mTextColor = ContextCompat.getColor(getAppContext(), R.color.color_627176);
                    mPageBg = ContextCompat.getColor(getAppContext(), R.color.color_001c27);
                    break;
            }
        }
    ...
    public void saveRecord() {
        ...
        //存储到数据库
        BookRecordHelper.getsInstance(mContext).saveRecordBook(mBookRecord);
    }
    ...
    public void openBook(CollBookBean collBook) {
        ...
        //从数据库取阅读数据
        mBookRecord = BookRecordHelper.getsInstance(mContext)
    ...
    boolean prevChapter() {
        ...
        if (mCurChapterPos - 1 < 0) {
            ToastUtils.show(mContext, "已经没有上一章了");
            return false;
        }
    ...
    boolean nextChapter() {
        ...
        if (mCurChapterPos + 1 >= mChapterList.size()) {
            ToastUtils.show(mContext, "已经没有下一章了");
            return false;
        }
...
//                });
    ...
    private boolean checkStatus() {
        if (mStatus == STATUS_LOADING) {
            ToastUtils.show(mContext, "正在加载中，请稍等");
            return false;
